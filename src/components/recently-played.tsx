  ...
  return (
    <section className="pb-4 flex">
      ...
      {isDesktop && (
        <ul
          ref={containerRef}
          className={`flex-grow grid gap-2  ${isNarrow ? 'grid-cols-2 grid-rows-4' : 'grid-cols-4 grid-rows-2'} `}
        >
          {loading ? (
            ...
            <>
              {recentlyPlayed.map((item, index) => (
                <li
                  key={index}
                  className="h-full rounded shadow bg-[#32312a] hover:bg-[#494941] overflow-hidden"
                >
