    ...
    useEffect(() => {
        const getRecommended = async () => {
            try {
                ...
                const topArtistsResponse = await fetchSpotify("me/top/artists?limit=10", accessToken);
                const topArtists = topArtistsResponse.items.slice(0, 10).map((artist: any) => artist.id);

                // Lấy danh sách 10 bài hát hàng đầu
                const topTracksResponse = await fetchSpotify("me/top/tracks?limit=10", accessToken);
            ...
            {isDesktop &&
                <>
                    <div className="flex items-center justify-between">
                        <h1 className=" text-white text-2xl font-bold mb-2">Gợi ý cho bạn</h1>
                        <div className="">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
                        {recommended.map((track: any) => (
                            <div key={track.id} className="flex flex-col items-center justify-between hover:bg-[#1b1b1b] rounded-md overflow-hidden cursor-pointer">
